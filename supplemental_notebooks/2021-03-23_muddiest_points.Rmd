...
1) Click the `download` button to put this Rmd on your computer.   
...
5) Click `Choose file` and find this file.   (`2021-03-23-_muddiest_points.Rmd`) on your computer.   
6) Click `OK`.   

Now you can open up this file and run it on your computer. 
...
Often PCA will be more informative if we only run it on a high variance subset of the genes. 
So we will use the top 500 genes in this example. 
...
`prcomp()` expects the features (in our case, genes) to be columns, but in our data and often gene expression matrices in general, genes are actually rows. 
...
Sometimes you will want to use the argument `scale = TRUE` but for Variance Stabilizing Transformed data, this is not necessary. 
...
The data in `$x` is a matrix where rows are samples and columns are the components.  
You will have as many components as you do samples. 
...
Let's plot the first PC components as we have elsewhere. 
...
For example, if you sets of samples that use different RNA selection strategies, looking at the loadings for PC1 may be helpful. 
If in a poly-A selected sample there high loadings for genes that are not poly-adenylated (e.g., a bunch of histones) or if a ribo-minus selected sample
has ribosomal RNA show up as the top hits this may indicate that RNA selection methods didn't thoroughly process. 

That being said, we'll take a look at how we can see this information. 
For example, what's the top 6 genes correlated to PC1? 
